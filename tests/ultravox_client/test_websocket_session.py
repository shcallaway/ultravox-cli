...
from typing import Any, Dict, Generator, Literal, cast, List, AsyncIterator, Optional, TypeVar, Generic
...
async def test_start(session: WebsocketSession) -> None:
    ...
    with patch("websockets.asyncio.client.connect", AsyncMock(return_value=mock_socket)):
        await session.start()
...
async def test_stop(session: WebsocketSession, mock_socket: MagicMock) -> None:
    ...
    session._state = cast(Literal["idle", "listening", "thinking", "speaking"], "listening")
...
async def test_socket_receive_normal_close(
    ...
    mock_socket.__aiter__.return_value = AsyncMock(__anext__=AsyncMock(side_effect=ws_exceptions.ConnectionClosedOK(None, None)))
